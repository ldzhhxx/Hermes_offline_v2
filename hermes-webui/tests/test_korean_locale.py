...
def test_korean_locale_includes_representative_translations():
    ...
    expected = [
        ...
        "onboarding_title: 'DiAgent Web UI에 오신 것을 환영합니다'",
    ]
...
def test_korean_settings_detail_descriptions_are_translated():
    ...
    expected = [
        ...
        "settings_desc_sync_insights: 'WebUI 토큰 사용량을 state.db에 반영하여 CLI /insights 명령에 브라우저 세션 데이터가 포함되도록 합니다. 기본값은 꺼짐입니다.'",
        "settings_desc_check_updates: 'WebUI 또는 Agent의 새 버전이 있으면 배너를 표시합니다. 백그라운드에서 주기적으로 git fetch를 실행합니다.'",
        "settings_desc_bot_name: 'UI 전체에 표시되는 Assistant 이름입니다. 기본값은 DiAgent입니다.'",
